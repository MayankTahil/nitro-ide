...
class MPSDouble(MPSDatatype):
	value =""
	constraints = []
	
	def __init__(self,_value):
		if _value :
			constraints = MPSConstraintDouble[MPSConstants.GENERIC_CONSTRAINT]
			length = len(constraints)
			self.constraints[i]= [ MPSConstraintDouble() for _ in range(length)]
			self.value =_value
