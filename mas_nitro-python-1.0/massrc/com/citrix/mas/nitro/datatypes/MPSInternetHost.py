...
class MPSInternetHost(MPSDatatype):
	...
	def __init__(self,_value):
		if _value :
			constraints = MPSConstraintInternetHost[MPSConstants.GENERIC_CONSTRAINT]
			length = len(constraints)
			self.constraints[i]= [ MPSConstraintInternetHost() for _ in range(length)]
			self.value =_value
