...
class MPSIPAddress6Net(MPSDatatype):
	...
	def __init__(self,_value):
		if _value :
			constraints = MPSConstraintIP6Net[MPSConstants.GENERIC_CONSTRAINT]
			length = len(constraints)
			self.constraints[i]= [ MPSConstraintIP6Net() for _ in range(length)]
			self.value =_value
