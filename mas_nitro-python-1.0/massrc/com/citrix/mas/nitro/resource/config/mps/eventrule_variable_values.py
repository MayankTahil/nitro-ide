...
class eventrule_variable_values(base_resource):
	_parent_name= ""
	_value= ""
	_name= ""
	_display_name= ""
	...
	def get_object_type(self) :
		try:
			return "eventrule_variable_values"
		except Exception as e :
			raise e
	...
	def file_component_value(self) :
		try :
			return "eventrule_variable_valuess"
		except Exception as e :
			raise e
	...
	def get(cls,client = None,resource="",option_=""): 
		try:
			response=""
			if not resource :
				eventrule_variable_values_obj=eventrule_variable_values()
				response = eventrule_variable_values_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this API to fetch filtered set of eventrule_variable_values resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return eventrule_variable_values_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the eventrule_variable_values resources.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = eventrule_variable_values_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of eventrule_variable_values resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = eventrule_variable_values_obj.getfiltered(service, option_)
			if response :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(eventrule_variable_values_response, response, self.__class__.__name__)
			if(result.errorcode != 0) :
				...
				if result.severity :
					...
					raise nitro_exception(result.errorcode, str(result.message), str(result.severity))
			return result.eventrule_variable_values
		except Exception as e :
			raise e
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(eventrule_variable_values_responses, response, "eventrule_variable_values_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.eventrule_variable_values_response_array
				i=0
				error = [eventrule_variable_values() for _ in range(len(response))]
				for obj in response :
			...
			response = result.eventrule_variable_values_response_array
			i=0
			eventrule_variable_values_objs = [eventrule_variable_values() for _ in range(len(response))]
			for obj in response :
				if hasattr(obj,'_eventrule_variable_values'):
					for props in obj._eventrule_variable_values:
						result = service.payload_formatter.string_to_bulk_resource(eventrule_variable_values_response,self.__class__.__name__,props)
						eventrule_variable_values_objs[i] = result.eventrule_variable_values
						i=i+1
			return eventrule_variable_values_objs
		except Exception as e :
			raise e
	...
	def validate(self,operationType):
		try:
			super(eventrule_variable_values,self).validate()
		except Exception as e :
			raise e
...
'''
class eventrule_variable_values_response(base_response):
	def __init__(self,length=1) :
		self.eventrule_variable_values= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.eventrule_variable_values= [ eventrule_variable_values() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class eventrule_variable_values_responses(base_response):
	def __init__(self,length=1) :
		self.eventrule_variable_values_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.eventrule_variable_values_response_array = [ eventrule_variable_values() for _ in range(length)]
