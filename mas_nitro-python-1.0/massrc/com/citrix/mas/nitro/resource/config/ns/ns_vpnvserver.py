...
class ns_vpnvserver(base_resource):
	_user_managed= ""
	_hostname= ""
	_state= ""
	_ns_ip_address= ""
	...
	'''
	get Vserver Current Users.
	'''
	@property
	def vsvr_cur_users(self) :
		try:
			return self._vsvr_cur_users
		except Exception as e :
			raise e


	'''
	get Vserver Maximum Users.
	'''
	@property
	def vsvr_max_users(self) :
		try:
			return self._vsvr_max_users
		except Exception as e :
			raise e
	...
	def get(cls,client = None,resource="",option_=""): 
		try:
			response=""
			if not resource :
				ns_vpnvserver_obj=ns_vpnvserver()
				response = ns_vpnvserver_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this API to fetch filtered set of ns_vpnvserver resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return ns_vpnvserver_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the ns_vpnvserver resources.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = ns_vpnvserver_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of ns_vpnvserver resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = ns_vpnvserver_obj.getfiltered(service, option_)
			if response :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(ns_vpnvserver_response, response, self.__class__.__name__)
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(ns_vpnvserver_responses, response, "ns_vpnvserver_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.ns_vpnvserver_response_array
				i=0
				error = [ns_vpnvserver() for _ in range(len(response))]
				for obj in response :
			...
			response = result.ns_vpnvserver_response_array
			i=0
			ns_vpnvserver_objs = [ns_vpnvserver() for _ in range(len(response))]
			for obj in response :
				if hasattr(obj,'_ns_vpnvserver'):
					for props in obj._ns_vpnvserver:
						result = service.payload_formatter.string_to_bulk_resource(ns_vpnvserver_response,self.__class__.__name__,props)
						ns_vpnvserver_objs[i] = result.ns_vpnvserver
						i=i+1
			return ns_vpnvserver_objs
		except Exception as e :
			raise e
	...
	def validate(self,operationType):
		try:
			super(ns_vpnvserver,self).validate()
		except Exception as e :
			raise e
...
'''
class ns_vpnvserver_response(base_response):
	def __init__(self,length=1) :
		self.ns_vpnvserver= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.ns_vpnvserver= [ ns_vpnvserver() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class ns_vpnvserver_responses(base_response):
	def __init__(self,length=1) :
		self.ns_vpnvserver_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.ns_vpnvserver_response_array = [ ns_vpnvserver() for _ in range(length)]
