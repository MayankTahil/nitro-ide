...
class ns_csvserver(base_resource):
	_user_managed= ""
	_curclntconnections= ""
	...
	def get(cls,client = None,resource="",option_=""): 
		try:
			response=""
			if not resource :
				ns_csvserver_obj=ns_csvserver()
				response = ns_csvserver_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this API to fetch filtered set of ns_csvserver resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return ns_csvserver_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the ns_csvserver resources.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = ns_csvserver_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of ns_csvserver resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = ns_csvserver_obj.getfiltered(service, option_)
			if response :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(ns_csvserver_response, response, self.__class__.__name__)
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(ns_csvserver_responses, response, "ns_csvserver_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.ns_csvserver_response_array
				i=0
				error = [ns_csvserver() for _ in range(len(response))]
				for obj in response :
			...
			response = result.ns_csvserver_response_array
			i=0
			ns_csvserver_objs = [ns_csvserver() for _ in range(len(response))]
			for obj in response :
				if hasattr(obj,'_ns_csvserver'):
					for props in obj._ns_csvserver:
						result = service.payload_formatter.string_to_bulk_resource(ns_csvserver_response,self.__class__.__name__,props)
						ns_csvserver_objs[i] = result.ns_csvserver
						i=i+1
			return ns_csvserver_objs
		except Exception as e :
			raise e
	...
	def validate(self,operationType):
		try:
			super(ns_csvserver,self).validate()
		except Exception as e :
			raise e
...
'''
class ns_csvserver_response(base_response):
	def __init__(self,length=1) :
		self.ns_csvserver= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.ns_csvserver= [ ns_csvserver() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class ns_csvserver_responses(base_response):
	def __init__(self,length=1) :
		self.ns_csvserver_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.ns_csvserver_response_array = [ ns_csvserver() for _ in range(length)]
