...
class ns_service(base_resource):
	_svr_name= ""
	_svc_state= ""
	_svc_port= ""
	_hostname= ""
	...
	'''
	Use this operation to enable the service.
	'''
	@classmethod
	...
	def get(cls,client = None,resource="",option_=""): 
		try:
			response=""
			if not resource :
				ns_service_obj=ns_service()
				response = ns_service_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this operation to disable the service.
	'''
	@classmethod
	...
	'''
	Use this API to fetch filtered set of ns_service resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return ns_service_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the ns_service resources.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = ns_service_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of ns_service resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = ns_service_obj.getfiltered(service, option_)
			if response :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(ns_service_response, response, self.__class__.__name__)
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(ns_service_responses, response, "ns_service_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.ns_service_response_array
				i=0
				error = [ns_service() for _ in range(len(response))]
				for obj in response :
			...
			response = result.ns_service_response_array
			i=0
			ns_service_objs = [ns_service() for _ in range(len(response))]
			for obj in response :
				if hasattr(obj,'_ns_service'):
					for props in obj._ns_service:
						result = service.payload_formatter.string_to_bulk_resource(ns_service_response,self.__class__.__name__,props)
						ns_service_objs[i] = result.ns_service
						i=i+1
			return ns_service_objs
		except Exception as e :
			raise e
	...
	def validate(self,operationType):
		try:
			super(ns_service,self).validate()
		except Exception as e :
			raise e
...
'''
class ns_service_response(base_response):
	def __init__(self,length=1) :
		self.ns_service= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.ns_service= [ ns_service() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class ns_service_responses(base_response):
	def __init__(self,length=1) :
		self.ns_service_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.ns_service_response_array = [ ns_service() for _ in range(length)]
