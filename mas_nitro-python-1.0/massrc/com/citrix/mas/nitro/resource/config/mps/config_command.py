...
class config_command(base_resource):
	_protocol= ""
	_index= ""
	_parent_name= ""
	_timeout= ""
	...
	'''
	Use this operation to add config commands.
	'''
	@classmethod
	def add(cls,service=None,resource=None):
		try:
			...
			if type(resource) is not list :
				...
				config_command_obj= config_command()
				return cls.perform_operation_bulk_request(service,"add", resource,config_command_obj)
		except Exception as e :
			raise e

	'''
	Use this API to fetch filtered set of config_command resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return config_command_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the config_command resources.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = config_command_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of config_command resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = config_command_obj.getfiltered(service, option_)
			if response :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(config_command_response, response, self.__class__.__name__)
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(config_command_responses, response, "config_command_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.config_command_response_array
				i=0
				error = [config_command() for _ in range(len(response))]
				for obj in response :
			...
			response = result.config_command_response_array
			i=0
			config_command_objs = [config_command() for _ in range(len(response))]
			for obj in response :
				if hasattr(obj,'_config_command'):
					for props in obj._config_command:
						result = service.payload_formatter.string_to_bulk_resource(config_command_response,self.__class__.__name__,props)
						config_command_objs[i] = result.config_command
						i=i+1
			return config_command_objs
		except Exception as e :
			raise e
	...
	def validate(self,operationType):
		try:
			super(config_command,self).validate()
		except Exception as e :
			raise e
...
'''
class config_command_response(base_response):
	def __init__(self,length=1) :
		self.config_command= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.config_command= [ config_command() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class config_command_responses(base_response):
	def __init__(self,length=1) :
		self.config_command_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.config_command_response_array = [ config_command() for _ in range(length)]
