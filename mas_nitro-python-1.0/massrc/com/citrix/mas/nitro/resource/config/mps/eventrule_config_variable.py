...
class eventrule_config_variable(base_resource):
	_parent_name= ""
	_name= ""
	_display_name= ""
	_description= ""
	...
	def get_object_type(self) :
		try:
			return "eventrule_config_variable"
		except Exception as e :
			raise e
	...
	def file_component_value(self) :
		try :
			return "eventrule_config_variables"
		except Exception as e :
			raise e
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(eventrule_config_variable_response, response, self.__class__.__name__)
			if(result.errorcode != 0) :
				...
				if result.severity :
					...
					raise nitro_exception(result.errorcode, str(result.message), str(result.severity))
			return result.eventrule_config_variable
		except Exception as e :
			raise e
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(eventrule_config_variable_responses, response, "eventrule_config_variable_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.eventrule_config_variable_response_array
				i=0
				error = [eventrule_config_variable() for _ in range(len(response))]
				for obj in response :
			...
			response = result.eventrule_config_variable_response_array
			i=0
			eventrule_config_variable_objs = [eventrule_config_variable() for _ in range(len(response))]
			for obj in response :
				if hasattr(obj,'_eventrule_config_variable'):
					for props in obj._eventrule_config_variable:
						result = service.payload_formatter.string_to_bulk_resource(eventrule_config_variable_response,self.__class__.__name__,props)
						eventrule_config_variable_objs[i] = result.eventrule_config_variable
						i=i+1
			return eventrule_config_variable_objs
		except Exception as e :
			raise e
	...
	def validate(self,operationType):
		try:
			super(eventrule_config_variable,self).validate()
		except Exception as e :
			raise e
...
'''
class eventrule_config_variable_response(base_response):
	def __init__(self,length=1) :
		self.eventrule_config_variable= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.eventrule_config_variable= [ eventrule_config_variable() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class eventrule_config_variable_responses(base_response):
	def __init__(self,length=1) :
		self.eventrule_config_variable_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.eventrule_config_variable_response_array = [ eventrule_config_variable() for _ in range(length)]
