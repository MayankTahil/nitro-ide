...
class rba_statement(base_resource):
	...
	def add(cls,service=None,resource=None):
		try:
			...
			if type(resource) is not list :
				...
				rba_statement_obj= rba_statement()
				return cls.perform_operation_bulk_request(service,"add", resource,rba_statement_obj)
		except Exception as e :
			raise e
		...
		try :
			...
			if type(resource) is not list :
				return resource.delete_resource(client)
					...
					rba_statement_obj=rba_statement()
					return cls.delete_bulk_request(client,resource,rba_statement_obj)
		except Exception as e :
			raise e
	...
	def get(cls,client = None,resource="",option_=""): 
		try:
			...
			if not resource :
				rba_statement_obj=rba_statement()
				response = rba_statement_obj.get_resources(client,option_)
			else:
	...
	def update(cls,client=None,resource=None):
		try:
			...
			if type(resource) is not list :
				...
				rba_statement_obj=rba_statement()
				return cls.update_bulk_request(client,resource,rba_statement_obj)
		except Exception as e :
			raise e

	'''
	Use this API to fetch filtered set of rba_statement resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return rba_statement_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the rba_statement resources.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = rba_statement_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of rba_statement resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = rba_statement_obj.getfiltered(service, option_)
			if response :
