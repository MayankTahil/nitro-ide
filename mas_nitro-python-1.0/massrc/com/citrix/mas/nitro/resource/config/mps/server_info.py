...
class server_info(base_resource):
	_report_end_time= ""
	_anomaly_data=[]
	_rpt_sample_time= ""
	_duration= ""
	...
	def get(cls,client = None,resource="",option_=""): 
		try:
			response=""
			if not resource :
				server_info_obj=server_info()
				response = server_info_obj.get_resources(client,option_)
			else:
	...
	'''
	Use this API to fetch filtered set of server_info resources.
	...
	def get_filtered(cls,service,filter_) :
		try:
			...
			return server_info_obj.getfiltered(service, option_)
		except Exception as e :
			raise e

	'''
	* Use this API to count the server_info resources.
	'''
	@classmethod
	def count(cls,service) :
		try:
			...
			option_ = options()
			option_._count=True
			response = server_info_obj.get_resources(service, option_)
			if response :
	...
	'''
	Use this API to count the filtered set of server_info resources.
	filter string should be in JSON format.eg: "vm_state:DOWN,name:[a-z]+"
	'''
	@classmethod
	def count_filtered(cls,service,filter_):
		try:
			...
			option_._count=True
			option_._filter=filter_
			response = server_info_obj.getfiltered(service, option_)
			if response :
	...
	def get_nitro_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(server_info_response, response, self.__class__.__name__)
	...
	def get_nitro_bulk_response(self,service ,response):
		try :
			result=service.payload_formatter.string_to_resource(server_info_responses, response, "server_info_response_array")
			if(result.errorcode != 0) :
				if (result.errorcode == 444) :
					service.clear_session(self)
				response = result.server_info_response_array
				i=0
				error = [server_info() for _ in range(len(response))]
				for obj in response :
			...
			response = result.server_info_response_array
			i=0
			server_info_objs = [server_info() for _ in range(len(response))]
			for obj in response :
				if hasattr(obj,'_server_info'):
					for props in obj._server_info:
						result = service.payload_formatter.string_to_bulk_resource(server_info_response,self.__class__.__name__,props)
						server_info_objs[i] = result.server_info
						i=i+1
			return server_info_objs
		except Exception as e :
			raise e
	...
	def validate(self,operationType):
		try:
			super(server_info,self).validate()
		except Exception as e :
			raise e
...
'''
class server_info_response(base_response):
	def __init__(self,length=1) :
		self.server_info= []
		self.errorcode = 0 
		self.message = "" 
		self.severity = "" 
		self.server_info= [ server_info() for _ in range(length)]
'''
Forms the proper response for bulk operation.
'''
class server_info_responses(base_response):
	def __init__(self,length=1) :
		self.server_info_response_array = []
		self.errorcode = 0 
		self.message = "" 
		self.server_info_response_array = [ server_info() for _ in range(length)]
